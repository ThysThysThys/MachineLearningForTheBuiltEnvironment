...
def plotter(xs,ys,zs,ts):
    ...
    ax.scatter(xs,ys,zs,color='orange', s=50,label='Waypoints')
    ax.plot(xs,ys,zs, color='green',linewidth=2,label='Trajectory')
    #ax.legend(loc='upper right')
    ...
    #for xc,yc,zc in zip(xs,ys,zs):
    for a,b,c,d in zip(xs,ys,zs,ts):
        ax.text(a+0.15,b+0.15,c+0.15, f"t={d}", fontsize=8,fontweight='bold')
